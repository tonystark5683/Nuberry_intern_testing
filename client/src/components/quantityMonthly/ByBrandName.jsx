...
const ByBrandName = ({ originalData, storeNameValue, departmentNameValue, productCategoryNameValue,handleBackButtonClick_Brand }) => {
  ...
  useEffect(() => {
    ...
    const groupData = () => {
      ...
      const groupedData = filteredData.reduce((accumulator, item) => {
        ...
        const monthKey = new Date(item.BillDate).toLocaleString('en-US', { month: 'long' });
    ...
    groupData();
  }, [originalData, storeNameValue, departmentNameValue, productCategoryNameValue]);
...
const handleSortClick = () => {
    ...
    const newSortedData = groupedData.slice().sort((a, b) => b.Amount - a.Amount);
  ...
  const uniqueBrandNames = [...new Set(groupedData.map((item) => item.BrandName))];
