...
const GoupedDataDepartment = ({
  ...
  const totalAmount = groupedData.reduce((sum,item)=> sum+Math.round(parseFloat(item.Amount)),0);
  ...
  const barChart = groupedData[0] ? (
    <Bar
      data={{
        ...
        datasets: [
          {
            data: top20Data.map((data) => Math.round(parseFloat(data.Amount))),
            label: "Amount",
      ...
      options={{
        ...
        plugins: {
          ...
          title: {
            display: true,
            text: "Total Forecasted Sales By Product Category",
            color: "white",
            font: { size: "10" },
          },
        },
