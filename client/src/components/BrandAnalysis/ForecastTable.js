...
//  };
 const [data, setData] = useState();
  ...
  const columns = [
    ...
    { field: 'ProductCategoryName', headerName: 'ProductCategoryName', flex: 1, type: 'string' },
  ...
  const barChart = groupedData[0] ? (
    <Bar
      data={{
        ...
        datasets: [
          {
            data: groupedData.map((data) => Math.round(parseFloat(data.Amount))),
            label: 'Amount',
  ...
  return (
    ...
    {condition ? (
          <GoupedDataStore originalData={originalData} labelvalue={labelvalue} onBackButtonClick={handleBackButtonClick}/>
        ) : (
          <Box
          mt="20px"
          display="grid"
