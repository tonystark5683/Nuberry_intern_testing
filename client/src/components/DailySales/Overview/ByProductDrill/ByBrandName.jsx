...
const BrandName = ({
  ...
  useEffect(() => {
    ...
    const groupData = () => {
      ...
      const groupedData = filteredData.reduce((accumulator, item) => {
        ...
        const key = `${storeKey}-${departmentKey}-${productCategoryKey}-${brandkey}-${year}-${month}-${day}`;
  ...
  groupedData.forEach((entry) => {
    const brand = entry.BrandName; // Assuming there is a "Department" field in your data
